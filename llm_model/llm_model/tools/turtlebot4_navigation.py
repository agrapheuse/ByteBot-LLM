...
class Turtlebot4Navigator(BasicNavigator):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            DockStatus, "/dock", self._dockCallback, qos_profile_sensor_data
        )
    ...
    def startFollowWaypoints(self, poses):
        ...
        while not self.isTaskComplete():
            ...
            if feedback and i % 5 == 0:
                print(
                    "Executing current waypoint: {0}/{1: <5}".format(
                        str(feedback.current_waypoint + 1), str(len(poses))
                    ),
                    end="\r",
                )
