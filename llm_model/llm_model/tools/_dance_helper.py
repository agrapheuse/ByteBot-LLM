...
class DanceCommandPublisher(Node):
    ...
    def timer_callback(self):
        ...
        if not self.ready:
            ...
            if self.wait_on_params:
                if self.params_future.done():
                    try:
                        ...
                        self.get_logger().info(
                            "Set Params Service response %r" % (response,)
                        )
                    except Exception as e:
                        self.get_logger().info(
                            "Set Params Service call failed %r" % (e,)
                        )
                else:
                ...
                self.ready = True
                self.get_logger().info(
                    "Finished params set, start dance at time %f"
                    % (current_time.nanoseconds / float(1e9))
                )
            ...
            # Check is subscribers are ready
                ...
                self.get_logger().info(
                    "Subscribers connected, send safety_override param at time %f"
                    % (current_time.nanoseconds / float(1e9))
                )
        ...
        for next_action in next_actions:
            if isinstance(next_action, Move):
                ...
                self.get_logger().info(
                    "Time %f New lights action, first led (%d,%d,%d)"
                    % (
                        current_time.nanoseconds / float(1e9),
                        lightring.leds[0].red,
                        lightring.leds[0].green,
                        lightring.leds[0].blue,
                    )
                )
            else:
                twist = Twist()
                ...
                self.finished = True
                self.get_logger().info(
                    "Time %f Finished Dance Sequence"
                    % (current_time.nanoseconds / float(1e9))
                )
                raise FinishedDance
    ...
    def send_params_request(self):
        safety_override = ParameterValue(
            type=ParameterType.PARAMETER_STRING, string_value="backup_only"
        )
